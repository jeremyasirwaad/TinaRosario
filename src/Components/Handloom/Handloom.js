		...
		const data = await fetch("http://products.tinarosario.com/api/Products")
			...
			.then((result) => {
				...
				const data = temp.filter(
					(e) => e.attributes.Main_Category == "Handloom Collections"
				);
				setPagedata(data);
				setLoading(false);
			});
	...
	return (
		<div className="designercollections">
			...
			<div className="prodpage">
				<span className="prodpaget1">Handloom Collections</span>
				<span className="prodpaget2">Specially crafted to don you</span>
				{loading ? (
					<div
						style={{
							display: "flex",
