...
export const Feedback = () => {
	...
	const changerating = (data) => {
		setRating(data);
		setformData({ ...formData, overall: data });
	};
	console.log(rating);
	...
	const fileonchange = (e) => {
		...
		const fil = files[0];
		if (
			fil.name.endsWith(".jpeg") ||
			fil.name.endsWith(".jpg") ||
			fil.name.endsWith(".png")
		) {
			setfile(fil);
	...
	const handleFormSubmit = async () => {
		...
		if (
			...
			toast.error("Please fill in all fields in Rating");
			return 0;
		}

		if (rating == 0) {
			toast.error("Please fill Overall Rating");
			return 0;
		}
				...
				const postImage = async () => {
					...
					try {
						const result = await axios.post(
							`${backendUrl}/feedback/photo/${data.data._id}`,
							formData,
							{
								headers: {
	...
	return (
		<div className="feedback_page">
			...
			<div className="feedbackinner">
				<span className="feedbackt1">Write a Feedback</span>
