...
export const OrdernowEthinic = () => {
  ...
  const secondhandler = () => {
    ...
    if (name == "" || email == "" || contactno == "" || !validator.isEmail(email)) {
      return;
    } else {
      placeorder();
    }
  ...
  //   const sendreq = await fetch("http://localhost:80/formData", {
  //     method: "POST",
  ...
  //     .then(async (res) => {
  //       console.log(res);
  //       const image = await axios.post(`http://localhost:80/formData/photo/:${res.id}`, file);
  //       if (image) {
  //         console.log(image);
  //       }
  //       if (res.status == true) {
  ...
  const placeorder = async () => {
    const sendData = await axios.post("http://localhost:80/formData", {
      gender: stylingfor,
      fabric: Fabric,
      clothingChoice: clothing,
      note: customnote,
    ...
    if (sendData) {
      ...
      const postImage = async () => {
        ...
        try {
          const result = await axios.post(`http://localhost:80/formData/photo/${sendData.data._id}`, formData, {
            headers: {
          ...
          if (result) {
            ...
            navigate("/ordernow/success/" + sendData.data._id);
          }
        } catch (error) {
          ...
          <div className="ordernowcontainer">
            ...
            >
              ...
              >
                ...
                >
                  {imginputloader ? <i style={{ color: "blue" }} class="fa-solid fa-check"></i> : "Upload Design"}
